...
public class ActionRecipe {
	...
	public ActionRecipe() {
		...
		setRecipe("le", new String[][]{{"eye_y","eye_p"}});
		setRecipe("ln", new String[][]{{"le","head_y","head_p"}});
		setRecipe("relln", new String[][]{{"le","head_y","head_p"}});
		setRecipe("lt", new String[][]{{"ln","body_y"}});
		setRecipe("speakto", new String[][]{{"ln","speak"}});
		...
		setRecipe("home", new String[][]{{"eye_y","eye_p","head_p","head_y","head_r","body_y","r_sho","r_elb","l_sho","l_elb"}});
		if(Main.robot_id ==2 ){
			setRecipe("induction", new String[][]{{"l_sho","l_elb","r_sho","r_elb"},{"l_sho","l_elb","r_sho","r_elb"}});
		}else{
			setRecipe("induction", new String[][]{{"leg"}});
		}
	}
	...
	public Recipe makeRecipe(String name){
			...
			for(int i=0;i<action_sequence.length;i++){
				...
				for(int j=0;j<action_tree.length;j++){
					...
					if(! recipe_map.containsKey(action_tree[j]) && !Main.Basic_motion_map.containsKey(action_tree[j]) && !nest.variable.containsKey(action_tree[j])){
						RobotUtil.Log(TAG,"レシピ合成失敗:"+action_name+" 素材不足:"+action_tree[j]);
						return null;
					}
